...
abstract class REST_Controller extends \CI_Controller
{
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            //check if key is set statically
            $row = new stdClass();
            if ($this->config->item('rest_use_static_key')) {
                if ($this->config->item('rest_static_key') != null && $key == $this->config->item('rest_static_key')) {
                    $row->{$this->config->item('rest_key_column')} = $this->config->item('rest_static_key');
                    $row->user_id = 0;
                    $row->level = 0;
                    $row->ignore_limits = 1;
                    $row->is_private_key = 1;
                    $row->ip_addresses = $_SERVER['REMOTE_ADDR'];
                }
                else return FALSE;
            } else {
